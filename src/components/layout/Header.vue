<template>
  <nav class="bg-white shadow">
    <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
      <div class="flex justify-between h-16">
        ...
        <div class="hidden lg:flex px-2 lg:px-0" v-if="user.enabled">
          <router-link v-if="!user.isSignedIn" to="/login"
            ...
            Log In </router-link><router-link v-if="!user.isSignedIn" to="/register"
          ...
          <button v-if="user.isSignedIn" @click="signOut"
            class="ml-4 inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out">
            Log Out
          </button>
          <router-link v-if="user.isSignedIn" to="/user" class="ml-4 inline-flex items-center px-1 pt-1">
            <span
    ...
    <div id="mobileMenu" class="lg:hidden" :class="{
      ...
      <div class="pt-2 pb-3">
        <router-link v-if="!user.isSignedIn && user.enabled" to="/register"
          class="mt-1 block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out">Register</router-link><router-link
          v-if="!user.isSignedIn && user.enabled" to="/login"
          class="mt-1 block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out">Log
          In</router-link>
        <router-link v-if="user.isSignedIn && user.enabled" to="/user"
          class="mt-1 block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out">Dashboard</router-link>
        <router-link v-if="user.isSignedIn && user.enabled" to="/logout"
          class="mt-1 block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out">Log
          Out</router-link>
        <router-link to="/c"
