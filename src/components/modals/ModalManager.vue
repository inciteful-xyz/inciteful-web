<template>
  <div v-if="validState" class="fixed inset-0 overflow-y-auto z-40">
    <div class="
                      ...
                      sm:p-0
                    ">
      <transition enter-active-class="transition ease-out duration-300" enter-class="opacity-0"
        ...
        <div v-show="validState" class="fixed inset-0 transition-opacity">
          <div class="absolute inset-0 bg-gray-500/75" v-on:click="clearModal()"></div>
        </div>
      </transition>
      ...
      <transition enter-active-class="ease-out duration-300"
        ...
        <div class="
                      ...
                      sm:max-w-4xl
                      sm:w-full
                      sm:p-6" role="dialog" aria-modal="true" v-show="validState" aria-labelledby="modal-headline">
          ...
          <div class="text-sm">
            <paper-modal-content v-if="isPaper" :options="(options as PaperModalOptions)" @clearModal="clearModal"
              @back="back" />
            <author-modal-content v-if="isAuthor" :options="(options as AuthorModalOptions)" @clearModal="clearModal"
              @back="back" />
          </div>
        </div>
      </transition>
