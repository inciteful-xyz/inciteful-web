<template>
  <div>
    <div v-if="paper">
      <paper-modal-button :text="paper.title" :id="paper.id" class="underline block font-semibold pb-2 text-left" />
      <div v-if="paper.author" class="font-semibold text-gray-500">
        <Authors :authors="paper.author" :ids="ids" />
      </div>
      <span>
        <i>{{ paper.journal }}</i>{{ paper.published_year && paper.journal ? ',' : '' }}
